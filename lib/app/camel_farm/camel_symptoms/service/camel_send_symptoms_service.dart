...
class CamelSendSymptomsService {
  static Future camelSendSymptomsService({
    required List<CamelAddModel> allData,
  }) async {
    ...
    final Uri url = Uri.parse(
        "${apiUrl}Camels/AddCamelSyndromes/${SharedPreferencesHelper.getCamelHerdValue()}");
...
/*
      request.send().then((res) {
        var sucessRes = res.stream.bytesToString();
        if (res.statusCode == 200 || res.statusCode == 201) {
          ...
          return 401;
        } else if (res.statusCode == 500 ||res.statusCode == 501 ||res.statusCode == 504) {
          return 500;
        }
        log("reeeeeeeees is : $res");
      });
      */
    } catch (e) {
      log("errrror : $e");
