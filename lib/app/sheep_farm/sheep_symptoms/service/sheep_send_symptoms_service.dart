...
class SheepSendSymptomsService {
  static Future sheepSendSymptomsService({
    required List<SheepAddModel> allData,
  }) async {
    ...
    final Uri url = Uri.parse(
        "${apiUrl}Sheeps/AddSheepSyndromes/${SharedPreferencesHelper.getCamelHerdValue()}");
    ...
    try {
      ...
      for (var y = 0; y < allData.length; y++) {
        if (allData[y].count.isNotEmpty) {
          //! i checked on list of count because when i call list of symptoms of api i equal count list with symptoms list that coming from api response
